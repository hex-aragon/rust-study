...
impl Calculator {
    ...
    fn math(&mut self, input: &str) -> i32 {
            ...
            match character {
                '+' => {
                    ...
                    if !self.current_input.is_empty() {
                        // 현재 입력이 비어있지 않다면
                        self.results.push(self.current_input.clone()); // 현재 입력을 결과 벡터에 추가
                        self.clear(); // 입력을 비워서 다음 숫자를 받을 준비
                    }
                ...
                '-' => {
                    if self.current_input.contains('-') || self.current_input.is_empty() {
                        ...
                        // 예: "-" 또는 "--"와 같은 경우
                        self.push_char(character); // 현재 문자(-)를 입력에 추가
                    } else {
                        // 숫자가 있는 경우 (예: "123-")
                        self.results.push(self.current_input.clone()); // 현재 입력을 결과 벡터에 추가
                        self.clear(); // 입력을 비움
                        self.push_char(character); // - 기호를 새로운 입력의 시작으로 추가
                    }
                }
                number => {
                    // 숫자를 만났을 때
                    if self.current_input.contains('-') {
                        ...
                        self.results.push(self.current_input.clone()); // 현재 입력을 결과 벡터에 추가
                        self.clear(); // 입력을 비움
                        self.push_char(number); // 새로운 숫자를 입력에 추가
                    } else {
                        // 마이너스가 없는 경우
                        self.push_char(number); // 현재 숫자를 입력에 추가
                    }
                }
            }
        }
        ...
        for entry in math_iter {
            ...
            // 숫자 처리
            if self.adds {
                self.total += entry.parse::<i32>().unwrap(); // 더하기
            } else {
                self.total -= entry.parse::<i32>().unwrap(); // 빼기
                self.adds = true; // 다음 연산을 더하기로 설정
            }
        }
        self.total // 최종 계산 결과 반환
    }
}
...
#[cfg(test)]
mod tests {
    use super::Calculator; // Calculator 구조체 사용

    // 1 + 1 = 2 테스트
    #[test]
    fn one_plus_one_is_two() {
        let mut calculator = Calculator::new();
        assert_eq!(calculator.math("1 + 1"), 2);
    }
    ...
    fn one_minus_two_is_minus_one() {
        let mut calculator = Calculator::new();
        assert_eq!(calculator.math("1 - 2"), -1);
    }
    ...
    // 9 + 9 - 9 - 9 = 0 테스트
    #[test]
    fn nine_plus_nine_minus_nine_minus_is_zero() {
        let mut calculator = Calculator::new();
        assert_eq!(calculator.math("9+9-9-9"), 0);
    }

    // 8 - 9 + 9 = 8 테스트 (끝에 추가 문자 포함)
    #[test]
    fn eight_minus_nine_plus_nine_is_eight_even_with_characters_on_the_end() {
        let mut calculator = Calculator::new();
        assert_eq!(calculator.math("8 -9 +9-----+++++"), 8);
    }
    ...
    fn panics_when_characters_not_right() {
        let mut calculator = Calculator::new();
        calculator.math("7 + please add seven");
    }
}