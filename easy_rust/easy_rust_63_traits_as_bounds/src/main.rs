...
fn attack_with_bow<T>(character: &T, opponent: &mut Monster, distance: u32) where 
...
where T: Magic + Debug,
{
    ...
    if distance < 15 {
        opponent.health -= 20;
        println!(
            "You raise your hands and cast a fireball. Your opponent now has {} health left. You are now at: {character:?}",
            opponent.health
        );
    }
}
